...
class InteractiveReportTests(unittest.TestCase):
    ...
    def test_single_report_is_deterministic_and_contains_direction_variants(self) -> None:
        ...
        self.assertIn("Drawdown depth × duration", first)
        self.assertIn("Profit correlation", first)
        self.assertIn('id="correlationFrequency"', first)
        self.assertIn('option value="daily"', first)
        self.assertIn('option value="weekly"', first)
        ...
        self.assertEqual(first.count('role="tabpanel"'), 11)
        self.assertEqual(first.count('data-tab-panel="'), 11)
        ...
        self.assertIn('href="#return-distributions"', first)
        self.assertIn('id="returnDistributionPanel"', first)
        self.assertIn('id="returnFrequency"', first)
        self.assertIn('.return-summary-table { width: 100%; min-width: 0; }', first)
        self.assertIn('function renderReturnDistributions()', first)
        self.assertIn("Return histogram", first)
        self.assertIn("function returnHistogramSvg(data)", first)
        self.assertIn("const binCount = Math.min(12, Math.max(1, observations.length));", first)
        self.assertIn("Periods (count)", first)
        self.assertIn("monthly-drawdown-table", first)
        self.assertIn("drawdownHeatStyle", first)
        self.assertIn("function niceStep", first)
        self.assertIn("function niceTicks", first)
        self.assertIn("Episode rank (sorted ascending)", first)
        self.assertIn("left.value - right.value", first)
        self.assertIn("one bar represents each episode", first)
        self.assertIn("function ddQuantile", first)
        self.assertIn("const percentileMarkers = [5, 50, 95]", first)
        self.assertIn("percentile-line", first)
        self.assertIn("Red dotted lines mark the historical P5, median, and P95 positions", first)
        self.assertIn("Vertical red dotted lines mark P5, median, and P95", first)
        self.assertIn('const markerSpecs = [["P5", stats.p5, 5], ["Median", stats.median, 50], ["P95", stats.p95, 95]];', first)
        self.assertIn("const rank = 1 + (observations.length - 1) * percentile / 100;", first)
        self.assertNotIn("Math.min(12, finite.length)", first)
        self.assertIn("grid-template-columns: 1fr", first)
        self.assertIn(".dd-chart-wrap { width: 100%; overflow: hidden; }", first)
        self.assertIn("function ddChartWidth()", first)
        self.assertIn("Math.floor(window.innerWidth - 86)", first)
        self.assertIn(".dd-chart-grid > * { min-width: 0; }", first)
        self.assertIn("form-action 'none';\">", first)
        self.assertNotIn("form-action 'none\">", first)
        self.assertNotIn("Math.round(absolute / 1000)}k", first)
        ...
        self.assertIn("drawdown_analysis", payload["variants"]["all"])
        self.assertIn("episodes", payload["variants"]["all"]["drawdown_analysis"])
    ...
    def test_report_without_monte_carlo_keeps_tab_available_with_guidance(self) -> None:
        ...
        self.assertIn("Ask the agent to run a Monte Carlo analysis", page)
        self.assertIn("regenerate the HTML report", page)
    ...
    def test_portfolio_payload_contains_members_and_daily_correlation(self) -> None:
        ...
        self.assertIn("drawdown_analysis", portfolio_view)
        self.assertIn("return_distributions", portfolio_view)
        self.assertIn("raw_drawdown_analysis", portfolio_view["members"]["left"]["analysis"])
        self.assertIn("allocated_drawdown_analysis", portfolio_view["members"]["left"]["analysis"])
        self.assertIn("raw_return_distributions", portfolio_view["members"]["left"]["analysis"])
        self.assertIn("allocated_return_distributions", portfolio_view["members"]["left"]["analysis"])
        self.assertIn("daily_profit", portfolio_view["correlations"])
        self.assertIn("weekly_profit", portfolio_view["correlations"])
        self.assertEqual(portfolio_view["correlations"]["weekly_profit"]["frequency"], "weekly")
